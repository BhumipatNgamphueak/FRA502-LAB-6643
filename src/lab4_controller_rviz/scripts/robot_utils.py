...
def find_xacro_file():
    """Auto-detect xacro file using ROS2 package system"""
    paths = []

    # Try using ament_index (installed packages)
    if AMENT_INDEX_AVAILABLE:
        try:
            pkg_share = get_package_share_directory('lab4_description')
            paths.extend([
                os.path.join(pkg_share, 'robot', 'visual', 'my-robot.xacro'),
                os.path.join(pkg_share, 'urdf', 'my-robot.xacro'),
            ])
        except Exception:
            pass

    # Fallback: Try source workspace paths
    ...
    paths.extend([
        ...
        f'{home}/FRA502-LAB-6643/src/lab4_description/robot/visual/my-robot.xacro',
        f'{home}/FRA502-LAB-6643/src/lab4_description/urdf/my-robot.xacro',
    ])
    ...
    raise FileNotFoundError(
        f"Cannot find xacro file. Searched paths:\n" +
