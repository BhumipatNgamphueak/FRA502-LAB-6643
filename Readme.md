...
1. **Clone the repository** (replace `StudentID` with your own ID):
   ...
   git clone -b LAB2 https://github.com/<your-org>/FRA502-LAB-StudentID.git
    ```
2. **Run turtlesim_plus_node** :
   ```bash
   ros2 run turtlesim_plus turtlesim_plus_node.py
   ```
3. **Run eater nodes** (replace `StudentID` with your own ID):
   ```bash
   ros2 run lab2 eater.py
   ```
4. **Run killer nodes** (replace `StudentID` with your own ID):
   ```bash
   ros2 run lab2 killer.py
   ```
5. **Run turtlesim_pose nodes** (replace `StudentID` with your own ID):
   ```bash
   ros2 run lab2 turtlesim_pose.py
   ```
5. set Maximum pizza (Replace <X> with Your maximum pizza)
    ```bash
   ros2 topic pub /set_max_pizza std_msgs/msg/Int16 'data: <X>'
   ```
7. Run Rviz2
    ```bash
   rviz2
